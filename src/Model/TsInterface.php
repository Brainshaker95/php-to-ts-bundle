...
final class TsInterface implements Stringable
{
     ...
     */
    private function getDocComment(array $generics): string
    {
        $templateTags = array_filter(array_map(
            static fn (TsGeneric $generic) => $generic->getTemplateTag(),
            $generics,
        ));

        if (!$this->description && !$this->deprecation && empty($templateTags)) {
            return '';
        }
        ...
        if (!empty($templateTags)) {
            if ($hasDescription || $this->deprecation) {
                $docComment .= $linePrefix . PHP_EOL;
            }

            foreach ($templateTags as $templateTag) {
                $docComment .= $linePrefix . $templateTag . PHP_EOL;
            }
        }
